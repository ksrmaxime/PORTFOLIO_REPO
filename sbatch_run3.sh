#!/bin/bash -l
#SBATCH --job-name=run3_ai_confirmed
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=02:00:00
#SBATCH --output=logs/run3_ai_confirmed_%j.out
#SBATCH --error=logs/run3_ai_confirmed_%j.err
...
RUN2_JOB_ID=61431046
INPUT="${OUTDIR}/laws_structure_with_ai_relevant_job${RUN2_JOB_ID}.parquet"
...
TEMP_RUN_DIR="data/output/run3_job${SLURM_JOB_ID}"
...
if [ "$SCORE" = "NA" ]; then
  RUN_DIR="data/output/run3_no_score_job${SLURM_JOB_ID}"
else
  SCORE_TAG=$(printf "%.2f" "$SCORE" | tr '.' 'p')
  RUN_DIR="data/output/run3_${SCORE_TAG}_job${SLURM_JOB_ID}"
fi
