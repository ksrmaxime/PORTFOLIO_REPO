...
from src import run_target_prompts
...
def parse_output(raw: str, decision_col: str, justif_col: str) -> dict:
    ...
    if decision is pd.NA:
        ...
        print(f"[RUN_TARGET PARSE FAIL] raw_head={head}")
...
def main() -> int:
    ...
    ap.add_argument("--target_code", required=True, choices=list(run_target_prompts.TARGET_DEFINITIONS))
    ...
    decision_col = args.decision_col or f"target_{args.target_code}"
    justif_col = args.justif_col or f"target_{args.target_code}_JUSTIF"
    ...
    def _build_prompt(row: pd.Series, text_col: str) -> str:
        return run_target_prompts.build_user_prompt(row, text_col=text_col)
    ...
    out = run_llm_dataframe(
        ...
        cfg=run_cfg,
        client=client,
        system_prompt=run_target_prompts.build_system_prompt(args.target_code),
    ...
    target_cols = [c for c in [decision_col, justif_col] if c in out.columns]
    base_cols = [c for c in out.columns if c not in target_cols]
    out = out[base_cols + target_cols]
