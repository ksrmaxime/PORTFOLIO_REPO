...
from src import run_inst_prompts
...
def parse_output(raw: str, decision_col: str, justif_col: str) -> dict:
    ...
    if decision is pd.NA:
        ...
        print(f"[RUN_INST PARSE FAIL] raw_head={head}")
...
def main() -> int:
    ...
    ap.add_argument("--instrument_code", required=True, choices=list(run_inst_prompts.INSTRUMENT_DEFINITIONS))
    ...
    decision_col = args.decision_col or f"instrument_{args.instrument_code}"
    justif_col = args.justif_col or f"instrument_{args.instrument_code}_JUSTIF"
    ...
    def _build_prompt(row: pd.Series, text_col: str) -> str:
        return run_inst_prompts.build_user_prompt(row, text_col=text_col)
    ...
    out = run_llm_dataframe(
        ...
        cfg=run_cfg,
        client=client,
        system_prompt=run_inst_prompts.build_system_prompt(args.instrument_code),
    ...
    inst_cols = [c for c in [decision_col, justif_col] if c in out.columns]
    base_cols = [c for c in out.columns if c not in inst_cols]
    out = out[base_cols + inst_cols]

    out.to_parquet(parquet_path, index=False)
    out.to_csv(csv_path, index=False)

    n_oui = int(out[decision_col].eq(True).sum())
    n_non = int(out[decision_col].eq(False).sum())
    n_na = int(out[decision_col].isna().sum())
    print(f"Saved: {parquet_path} and {csv_path}")
    print(f"{decision_col} — OUI: {n_oui:,} | NON: {n_non:,} | NA (parse fail): {n_na:,}")
    return 0
