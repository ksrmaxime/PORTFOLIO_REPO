...
#SBATCH --output=logs/run1_relart_%j.out
#SBATCH --error=logs/run1_relart_%j.err
...
GOLD_CSV="data/external/RUN1_GOLD.csv"
...
if [ "$SCORE" = "NA" ]; then
  RUN_DIR="data/output/run1_no_score_job${SLURM_JOB_ID}"
else
  SCORE_TAG=$(printf "%.2f" "$SCORE" | tr '.' 'p')
  RUN_DIR="data/output/run1_${SCORE_TAG}_job${SLURM_JOB_ID}"
fi
