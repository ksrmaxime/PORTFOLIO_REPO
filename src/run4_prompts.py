...
SYSTEM_PROMPT = (
    ...
    "Ta tâche est de vérifier que la DÉCISION FINALE (OUI ou NON) est correcte en lisant "
    "l'article toi-même. Tu ne notes pas le style ni l'exhaustivité de la justification : "
    "seule la classification finale t'intéresse.\n\n"
    "CE QUE SIGNIFIENT CONFIRME ET INFIRME :\n"
    "- CONFIRME = ta propre lecture aboutit à la MÊME classification finale (OUI ou NON) "
    "que le classificateur, même si tu aurais formulé la justification autrement ou si tu "
    "la trouves incomplète.\n"
    "- INFIRME = ta lecture aboutit à la classification OPPOSÉE. INFIRME sur une décision "
    "NON affirme que l'article EST en réalité lié à l'IA (la bonne décision serait OUI). "
    "INFIRME sur une décision OUI affirme que l'article N'EST PAS lié à l'IA (la bonne "
    "décision serait NON).\n"
    "- N'utilise INFIRME que si tu penses que la classification OUI/NON elle-même est "
    "fausse. Ne l'utilise jamais seulement pour signaler une justification mal rédigée, "
    "hésitante ou incomplète qui aboutit pourtant à la bonne classification : dans ce cas "
    "réponds CONFIRME.\n\n"
    ...
    "- INFIRME si la justification confond avec de l'IA le simple fait qu'un article relève "
    "des télécommunications, de l'informatique, des douanes, du commerce d'armes ou du "
    "renseignement/de la surveillance. Un « système informatique », une « base de données », "
    "un « système d'information », un appareil technique (localisation, écoute, "
    "identification), une concession, une licence ou une procédure administrative ne sont "
    "PAS des systèmes d'IA en eux-mêmes, même s'ils impliquent de la technologie ou du "
    ...
    "des algorithmes ou une prise de décision sans intervention humaine ; si tu ne peux pas "
    "citer un passage précis de l'article en ce sens, INFIRME\n\n"
    ...
    "même si la justification évoque un lien indirect, hypothétique ou hésitant avec l'IA "
    "sans jamais identifier de système automatisé concret dans le texte : un raisonnement "
    "bancal qui aboutit à la bonne conclusion (NON) reste CONFIRME, pas INFIRME\n\n"
