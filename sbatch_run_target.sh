...
# défaut que sbatch_run_inst.sh). Une fois les 12 étapes terminées, lancer
# manuellement sbatch_run_inst.sh en lui passant en argument le job id SLURM de
# cette dernière étape (INFORMATION_SOCIETAL_HARMS) : il filtrera automatiquement
...
TARGETS=(RESEARCH_INNOVATION SKILLS_HUMAN_CAPITAL DATA_ACCESS_RESOURCES COMPUTE_INFRASTRUCTURE ADOPTION_DIFFUSION EXPERIMENTATION_MARKET DATA_PRIVACY IP_CREATIVE_RIGHTS SECURITY_ROBUSTNESS ACCOUNTABILITY_TRANSPARENCY HIGH_STAKES_RIGHTS INFORMATION_SOCIETAL_HARMS)
N_TARGETS=${#TARGETS[@]}
